...
class NetworkApiProvider {
  ...
  static Future<bool> postRegisterNetwork(
    ...
    try {
      ...
      Dialogs.alert(context,
          title: "Ocurrio un error", message: "La solicitud ya fué enviada");
      return false;
    }
  }
  ...
  static Future<bool> putNetworkRequestResponse(
    ...
    try {
      FormData formData = new FormData.fromMap(netReqEntity.toJson());
      await httpManager.putForm(
          "mobile/network-request-response/${netReqEntity.userBaseId}/${netReqEntity.userRelationId}",
          formData);
      return true;
    } catch (error) {
      Dialogs.alert(context,
          title: "Error", message: "No se pudo aceptar la solicitud");
      return false;
    }
  }
  ...
  static Future<List<ExpertEntity>?> getNetworkExpert() async {
    ...
    if (session != null) {
      ...
      try {
        final responseData =
            await httpManager.get("mobile/network-request/${userId}");
        return ExpertEntity.fromJSONList(responseData["data"]);
      } catch (e) {
        return null;
      }
  ...
  static Future<List<ExpertEntity>?> getNetworkRequestExpert() async {
    ...
    if (session != null) {
      ...
      try {
        final responseData =
            await httpManager.get("mobile/network-request-expert/${userId}");
        return ExpertEntity.fromJSONList(responseData["data"]);
      } catch (e) {
        return null;
      }
